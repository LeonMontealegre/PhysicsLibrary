...
public:
    // Constructors
    Matrix(const T& vals_ = T(0)): MatrixDyn<T>(N, M, vals_) {}
    Matrix(const std::initializer_list<std::initializer_list<T>>& vecs_): MatrixDyn<T>(vecs_) {
        if (MatrixDyn<T>::M != M || MatrixDyn<T>::N != N)
            throw 1; // `vecs_` has size != templated N,M
    }
};
